...
def update_re_sa_history():
    ...
    try:
        ...
        existing_dates = set(r[0] for r in db.query(ReSaData.date).all())
        new_records = [
            ReSaData(date=row["date"], value=row["value"], re_sa_change=row["re_sa_change"])
            for _, row in df.iterrows() if row["date"] not in existing_dates
        ]
        if new_records:
            db.bulk_save_objects(new_records)
            db.commit()
        return {"message": f"Retail Sales data updated. Inserted {len(new_records)} new records."}
    except Exception as e:
        raise HTTPException(500, detail=str(e))
    finally:
        db.close()